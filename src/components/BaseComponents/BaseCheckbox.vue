<template>
  <div>
    <span :class="{ 'ant-checkbox': true, 'ant-checkbox-checked': isChecked }">
      <input
        type="checkbox"
        :class="{ 'ant-checkbox-input': true, itemFinish: isChecked }"
        value="Apple"
        v-model="isChecked"
      />
      <span class="ant-checkbox-inner cc_pointer"> </span>
    </span>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      isChecked: this.checkedStatusProps
    };
  },
  computed: {},
  watch: {
    isChecked: function() {
      ...
      this.$emit("getCheckboxStatusEvent", this.isChecked);
    }
  },
  created() {},
  methods: {}
